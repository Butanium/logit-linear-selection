"""Judge all samples in parallel via `claude -p haiku`. Resumable — skips already-judged.

Reads samples from --workspace/samples/, writes judgments to --workspace/judgments/.
Judging criteria + schema are loaded from --judging-dir (default: ./judging/ next to this script).
"""
import argparse
import json
import subprocess
...
def main():
    ...
    ap.add_argument("--filter", default="*.txt", help="Glob over samples/ to subset what runs.")
    args = ap.parse_args()
